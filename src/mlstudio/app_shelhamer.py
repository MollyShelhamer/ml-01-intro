...
def make_prior_vs_final_score_chart(df_clean: pd.DataFrame) -> Axes:
    """Create a double line chart comparing prior score and final score."""
    LOG.info("Creating chart: prior score vs final score")

    fig, ax = plt.subplots(figsize=(10, 5))

    observations = range(len(df_clean))

    ax.plot(
        observations,
        df_clean["prior_score"],
        marker="o",
        linewidth=2,
        label="Prior Score",
        color="steelblue",
    ...
    ax.set_title("Prior Score vs Final Score")
...
def make_plots(df_clean: pd.DataFrame, model: LinearRegression) -> None:
    ...
    make_prior_vs_final_score_chart(df_clean)
